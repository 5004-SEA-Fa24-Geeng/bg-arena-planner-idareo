...
[//]: # ( [MermaidChart: e9d633d9-0628-4d0f-bcf6-7e23f19367ef]
---
title: bg_arena_planner - Final design
---
classDiagram
direction BT
class BGArenaPlanner {
...
+ filter(String filter, GameData sortOn, boolean ascending) : Stream~BoardGame~
+ reset() : void
+ getComparator(GameData sortOn): Comparator~BoardGame~
+ getGames()
}
class GameList {
- gameNames: Set~String~
...
- processHeader(String header) : Map~GameData, Integer~
    ...
    class GameDataSorts{
        + NameSort: class
        + RatingSort: class
        + DifficultySort: class
        + RankSort: class
        + MinPlayersSort: class
        + MaxPlayersSort: class
        + MinPlayTimeSort: class
        + MaxPlayTimeSort: class
        + YearSort: class
...
Prior to designing, I did not necessarily think that I would need too many additional methods or classes as I thought most of the operations could be handled within the current classes provided.
The main changes I made were how I handled the sorting in the planner class. I found that using a list of the filtered boardgames was helpful for chainedFilters compared to the stream I originally returned.
Another critical change I made was how I handled the sorting. I found that creating a separate class for sorts allowed me to isolate multiple comparators that could then
be used in the planner class to sort by column. This was all handled in a new method I did not know I needed. However, breaking up my sorts allowed me to isolate 
and easily Identify the areas of the code that were causing issues.